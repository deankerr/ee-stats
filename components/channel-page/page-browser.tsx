...
export function PageBrowser({ channel, page }: { channel: string; page: number }) {
  const logEntries = useQuery(api.queries.getPage, { channel, offset: page, numItems: 80 })
          ...
          {logEntries?.map((item, i) => {
            return (
              <LogEntryLine
                ...
                className={cn('flex snap-start py-0.5 hover:bg-muted/50')}
              />
            )
          })}
        </div>
